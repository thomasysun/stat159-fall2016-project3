...
We obtain the test mean squared errors for every predictive model from our analysis. Below is the table of MSEs for each model.
...
We look the MSE of each model, this time with post-graduate income as our regressor.
...
From these results we learn that coming from a low income bracket and having federal loans 
