...
xtable_summary <- xtable(ols_completion_summary, 
...
We find the $\lambda$ for the best model: best $\lambda$ = . Then, we use the test set to compute the test Mean Square Error: test MSE = . Finally we refit the model on the full data set using the best lambda and get official coefficients.
...
Once finding the number of principal components considered for the best model, we use the test set to compute the test Mean Square Error and finally refit the model on the full data set using the best number of components and get official coefficients.

\begin{figure}[!hb]
  \centering
    \includegraphics[width=70mm]{../../images/total_completion.png}
...
\subsection*{Post-Graduate Income}

Next, we looked determinants of income post-graduation. The following is a table showing the OLS regression coefficients for our benchmark.

<<xtable, results=tex, echo=FALSE>>=
xtable_summary <- xtable(ols_income_summary, 
                         caption = 'Summary Table of OLS Regression for Income')
print(xtable_summary, comment=FALSE, type = "latex", max.levels=0, caption.placement = 'top')
@
